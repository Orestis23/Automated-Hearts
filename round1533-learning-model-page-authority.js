/* Automated Hearts Round 1533 — authoritative Learning Center model controller.
...
(()=>{
  'use strict';
  if(window.__AH1533LearningModelAuthority) return;
  window.__AH1533LearningModelAuthority=1;
  /* Round 1546: this controller owns Learning Center stage geometry/return. */
  window.__AH_R1071_MODEL_CONTROLLER=1;
  window.__ahSharedModelReturnRound602=1;
  window.__AH1546LearningTransitionAuthority=1;
  ...
  const BG='./assets/page-shield-smoked-heart.webp?v=1528r';
  ...
  let learningFlowSpacer=null;
  const imp=(el,p,v)=>el&&el.style.setProperty(p,v,'important');
  const ensureLearningFlowSpacer=()=>{
    if(state.page!=='learning'||!state.stage||!matchMedia('(min-width:801px)').matches)return null;
    const choices=document.getElementById('learning-route-buttons');
    if(!choices)return null;
    if(!learningFlowSpacer||!learningFlowSpacer.isConnected){
      learningFlowSpacer=document.createElement('div');
      learningFlowSpacer.className='ah1546-learning-flow-spacer';
      learningFlowSpacer.setAttribute('aria-hidden','true');
      state.stage.parentNode?.insertBefore(learningFlowSpacer,state.stage);
    }
    const rect=choices.getBoundingClientRect();
    const h=Math.max(1,Math.round(rect.height||Math.max(560,window.innerHeight-54)));
    for(const [prop,val] of [['display','block'],['visibility','hidden'],['pointer-events','none'],['width','100%'],['height',`${h}px`],['min-height',`${h}px`],['max-height',`${h}px`],['margin','0'],['padding','0'],['border','0'],['overflow','hidden'],['flex','0 0 auto']]) imp(learningFlowSpacer,prop,val);
    return learningFlowSpacer;
  };
  const clearLearningFlowSpacer=()=>{if(learningFlowSpacer?.isConnected)learningFlowSpacer.remove();learningFlowSpacer=null;};
  ...
  const showStage=()=>{
    ...
    /* Round 1546: on Learning Center, do not toggle ah-model-stage-locked at click.
       That class switches the chooser between absolute and normal flow and was the
       visible first-frame jerk. Keep the chooser geometry fixed and reserve its exact
       flow height with an invisible spacer while the model stage is open. */
    if(state.page==='learning') ensureLearningFlowSpacer();
    else document.body.classList.remove('ah-model-stage-locked');
  ...
  const activate=(n,{scroll=false,force=false,direction='' }={})=>{
    ...
    if(frame){
      ...
      for(const [p,v] of [['display','block'],['visibility','visible'],['opacity',frame.dataset.ah1533Loaded==='1'?'1':'0'],['pointer-events','auto'],['transform','none']])imp(frame,p,v);
    ...
    [60,180,420,900,1600,2800].forEach(ms=>setTimeout(()=>{
      ...
      if(frame)for(const [p,v] of [['display','block'],['visibility','visible'],['opacity',frame.dataset.ah1533Loaded==='1'?'1':'0'],['pointer-events','auto'],['transform','none']])imp(frame,p,v);
  ...
  const openStageTransition=async()=>{
    ...
    try{
      ...
      /* Round 1545: never hard-reset scrollTop after the lesson button is pressed.
         With scroll anchoring frozen, the stage can enter layout without any corrective
         snap before the single requestAnimationFrame scroll trajectory begins. */
      await afterTwoFrames();
  ...
  const hideStage=async()=>{
    ...
    try{
      ...
      await afterTwoFrames();
      /* 2) Only after the shield is fully closed, scroll all the way to page top. */
      await smoothToPageTop(RETURN_SCROLL_MS);
      /* 3) Collapse the lower stage after the viewport is already back at the top. */
      document.body.classList.remove('r1073-model-open','r1075-model-open','r1076-model-open','r1079-model-open');
      /* The Learning chooser stayed locked for the whole trip, so there is no
         end-of-return layout-mode switch to snap back into place. */
      ...
      if(state.page==='learning') clearLearningFlowSpacer();
    }finally{
      delete document.body.dataset.ahStageTransition;
      restoreRootAnchor();
      window.dispatchEvent(new Event('automated-hearts:stage-transition-end'));
