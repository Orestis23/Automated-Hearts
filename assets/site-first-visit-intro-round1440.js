/* Automated Hearts Round 1440 — site-wide first-visit intro runtime with mechanical keys + shield-top audio event. */
(() => {
  'use strict';
  ...
  const boot = () => {
    ...
    const waitForMobileShellPaint = async () => {
      if (!window.matchMedia('(max-width:900px)').matches) return;
      const selector = 'body > .shell-edge.t, body > .shell-edge.b, body > .shell-edge.l, body > .shell-edge.r, body > .heart, body > .page-chip, body > .message, body > nav.footer';
      for (let tries = 0; tries < 180; tries += 1) {
        const nodes = document.querySelectorAll(selector);
        if (nodes.length >= 8 && document.querySelector('body > nav.footer')) break;
        await sleep(16);
      }
      await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    };

    waitForMobileShellPaint().then(story).then(finish).catch(async () => {
      try { localStorage.setItem(KEY, '1'); } catch (_) {}
      intro.classList.add('is-raising');
      await sleep(700);
      intro.hidden = true;
