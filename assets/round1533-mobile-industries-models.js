(()=>{
  ...
  async function openIndustry(){
    const entryScroller=scroller(), entryScrollTop=entryScroller.scrollTop;
    if(transitioning||activeIndustry===null||!stage||!modelShell)return;
    transitioning=true;document.body.dataset.ahStageTransition='1';document.body.dataset.ahModelHalf='transition-in';
    primeStageHeight();
    ...
    setShieldClosed();
    entryScroller.scrollTop=entryScrollTop;
    await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
    try{const finished=await animateScroll(stageTopTarget(),SCROLL_DOWN_MS);if(finished===false)return;document.body.dataset.ahModelHalf='bottom';warm(activeIndustry);showCurrent();const active=[...frameCache.values()].find(frame=>frame.dataset.ahActive==='1');if(active&&active.dataset.ah1533Loaded!=='1')await Promise.race([new Promise(resolve=>active.addEventListener('load',resolve,{once:true})),new Promise(resolve=>setTimeout(resolve,1250))]);await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));showCurrent();await lowerShield();}finally{delete document.body.dataset.ahStageTransition;window.dispatchEvent(new Event('automated-hearts:stage-transition-end'));transitioning=false;}
  }
  ...
  back?.addEventListener('click',async event=>{
    event.preventDefault();if(transitioning)return;back.disabled=true;transitioning=true;document.body.dataset.ahStageTransition='1';document.body.dataset.ahModelHalf='transition-out';
    try{pauseAll();await Promise.all([raiseShield(),animateScroll(0,SCROLL_UP_MS)]);await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));if(modelShell)modelShell.hidden=true;if(stage){stage.hidden=true;stage.setAttribute('aria-hidden','true');}activeIndustry=null;modelIndex=0;document.body.dataset.ahModelHalf='top';setShieldClosed();document.querySelector('[data-industry]')?.focus({preventScroll:true});}finally{delete document.body.dataset.ahStageTransition;window.dispatchEvent(new Event('automated-hearts:stage-transition-end'));transitioning=false;back.disabled=false;}
  });
  document.addEventListener('visibilitychange',()=>{if(document.hidden)pauseAll();else if(activeIndustry!==null&&document.body.dataset.ahModelHalf==='bottom')showCurrent();});
