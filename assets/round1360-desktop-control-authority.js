...
(function(){
  ...
  function setFooter(a,down){
    if(!a)return;
    imp(a,'background-color','transparent');imp(a,'background-image','url("'+(down?FOOT_DOWN:FOOT_REST)+'")');imp(a,'background-position','center');imp(a,'background-size','100% 100%');imp(a,'background-repeat','no-repeat');imp(a,'border','0');imp(a,'outline','0');imp(a,'filter',down?'brightness(.96)':'none');a.style.removeProperty('transform');a.setAttribute('data-ah-key-down',down?'1':'0');imp(a,'box-shadow',down?'inset 0 4px 7px rgba(0,0,0,.30), inset 0 1px 0 rgba(255,255,255,.035), 0 1px 2px rgba(0,0,0,.28)':'0 5px 7px rgba(255,46,168,.13)');
    var copy=a.querySelector('.send-control-copy');if(copy){copy.style.removeProperty('transform');imp(copy,'filter',down?'brightness(.94)':'none');}
  ...
  function bindMessage(el){if(!el||el.dataset.ahR1360Message==='1')return;el.dataset.ahR1360Message='1';el.addEventListener('pointerdown',function(){if(matchMedia(MQ).matches)setMessage(el,true);},{passive:true});el.addEventListener('pointerup',function(){if(matchMedia(MQ).matches)setTimeout(function(){setMessage(el,messageOpen(el));},120);},{passive:true});el.addEventListener('pointercancel',function(){if(matchMedia(MQ).matches)setMessage(el,messageOpen(el));},{passive:true});el.addEventListener('keydown',function(e){if(matchMedia(MQ).matches&&(e.key==='Enter'||e.key===' '))setMessage(el,true);});el.addEventListener('keyup',function(){if(matchMedia(MQ).matches)setTimeout(function(){setMessage(el,messageOpen(el));},80);});try{new MutationObserver(function(){if(matchMedia(MQ).matches)setMessage(el,messageOpen(el));}).observe(el,{attributes:true,attributeFilter:['aria-expanded','class']});new MutationObserver(function(){if(matchMedia(MQ).matches)setMessage(el,messageOpen(el));}).observe(document.documentElement,{attributes:true,attributeFilter:['class']});}catch(_e){}
  }
  function apply(){if(!matchMedia(MQ).matches)return;document.querySelectorAll('footer#site-footer nav#primary-nav a.footer-structure-control.mechanical-send-control[data-nav]').forEach(function(a){setFooter(a,false);bindFooter(a);});var msg=document.getElementById('header-send-message');if(msg){setMessage(msg,messageOpen(msg));bindMessage(msg);}}
